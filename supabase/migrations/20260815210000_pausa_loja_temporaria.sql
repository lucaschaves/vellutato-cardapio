-- Pausa temporária com término automático (pausado_ate).
-- pausado = true e pausado_ate nulo = pausa até reabrir manualmente.
-- pausado = true e pausado_ate no futuro = pausa até aquele instante.

alter table public.loja_config
  add column if not exists pausado_ate timestamptz;

comment on column public.loja_config.pausado_ate is
  'Se preenchido, a pausa acaba sozinha neste instante (timestamptz).';
