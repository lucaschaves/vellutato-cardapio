-- Retirada de evento com horário, dentro do funcionamento da loja.

drop function if exists public.criar_pedido_evento(
...
begin
  ...
  if p_horario is null or p_horario !~ '^\d{2}:\d{2}$' then
    raise exception 'FORA_HORARIO: Informe o horário da retirada.';
  end if;
  v_hora := p_horario::time;

  select h.aberto, h.abre::time, h.fecha::time
    into v_aberto, v_abre, v_fecha
  from public.loja_horarios h
  where h.dia_semana = extract(dow from p_data_retirada)::int;

  if not found or not coalesce(v_aberto, false) then
    raise exception 'FORA_HORARIO: A loja não abre neste dia.';
  end if;

  if v_abre < v_fecha then
    if v_hora < v_abre or v_hora >= v_fecha then
      raise exception 'FORA_HORARIO: Escolha um horário entre % e %.',
        to_char(v_abre, 'HH24:MI'), to_char(v_fecha, 'HH24:MI');
    end if;
  elsif v_hora < v_abre and v_hora >= v_fecha then
    raise exception 'FORA_HORARIO: Escolha um horário dentro do funcionamento da loja.';
  end if;

  if p_data_retirada = (now() at time zone 'America/Sao_Paulo')::date
     and v_hora <= (now() at time zone 'America/Sao_Paulo')::time then
    raise exception 'FORA_HORARIO: Esse horário já passou.';
  end if;
  ...
  insert into public.pedidos (
    ...
    (p_data_retirada::timestamp + v_hora) at time zone 'America/Sao_Paulo',
    jsonb_build_object(
...
grant execute on function public.criar_pedido_evento(
  text, text, uuid, text, text, date, text, integer, text, jsonb, numeric, numeric, numeric
