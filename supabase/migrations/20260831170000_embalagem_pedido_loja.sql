-- Embalagem / serviço para comer na loja (prato, talheres, enfeite).
-- Mesmo modelo de sacola: ceil(itens com modo_consumo=loja / N).

alter table public.loja_config
  add column if not exists ficha_embalagem_pedido_loja_id uuid
    references public.fichas_tecnicas (id) on delete set null,
  add column if not exists capacidade_embalagem_pedido_loja integer
    not null default 1 check (capacidade_embalagem_pedido_loja > 0);
...
begin
  ...
  loop
    ...
    if coalesce(item_row.modo_consumo, '') = 'loja' then
      v_itens_loja := v_itens_loja + item_row.quantidade;
  ...
  if v_cfg.id is not null then
    ...
    if v_itens_loja > 0 then
      v_n := coalesce(v_cfg.capacidade_embalagem_pedido_loja, 1);
      v_ficha_pedido := v_cfg.ficha_embalagem_pedido_loja_id;
      if v_ficha_pedido is not null and v_n is not null and v_n > 0 then
        v_sacolas := ceil(v_itens_loja / v_n::numeric);
        perform public.acumular_consumo_ficha(v_ficha_pedido, v_sacolas);
      end if;
    end if;
  end if;
