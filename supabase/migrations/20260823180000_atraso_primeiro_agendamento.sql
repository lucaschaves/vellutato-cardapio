-- Atraso (min) após a abertura antes do primeiro slot agendável.
-- Ex.: abre 14:00 + 15 → primeiro horário oferecido 14:15.

alter table public.loja_config
  add column if not exists atraso_primeiro_agendamento_min integer not null default 15
  check (atraso_primeiro_agendamento_min >= 0 and atraso_primeiro_agendamento_min <= 180);

comment on column public.loja_config.atraso_primeiro_agendamento_min is
  'Minutos após a abertura em que o primeiro horário de agendamento pode ser oferecido ao cliente.';
...
begin
  ...
  if v_horario.abre < v_horario.fecha then
    -- Dia normal: slot >= abre+atraso e < fecha
    v_ok := v_hora >= v_primeiro_slot and v_hora < v_horario.fecha;
  else
    -- Atravessa meia-noite (ex. 18→02): atraso só vale no trecho após a abertura
